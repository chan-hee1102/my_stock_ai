...
def get_client():
    key = st.secrets.get("GEMINI_API_KEY") # 절대 코드에 키를 직접 적지 마세요!
...
if data is not None:
    ...
    with col_chat:
        ...
        # AI 채팅 로직 (gemini-flash-latest 사용)
        if prompt := st.chat_input(f"{stock['종목명']}에 대해 질문하세요"):
            ...
            if client:
                try:
                    ...
                    # 유출된 키 등 상세 오류를 채팅창에 출력
                    st.error(f"⚠️ 오류 발생: {str(e)}")
            else:
                st.warning("⚠️ API 키가 설정되지 않았습니다. Streamlit Secrets를 확인하세요.")
            st.rerun()