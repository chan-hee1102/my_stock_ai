...
def get_official_news_analysis(stock_name, code):
    ...
    try:
        ...
        prompt = (f"{stock_name}의 최근 뉴스: {titles}\n\n"
                  f"위 뉴스 중 기업 가치에 중요한(수주, 계약, 신사업 등) 오피셜 뉴스 3개만 엄선하세요.\n"
                  f"시세 단순 변동 뉴스는 제외하고, '뉴스제목 | 핵심요약' 형식으로만 답변하세요.")
        res_ai = client.chat.completions.create(
            model="llama-3.3-70b-versatile",
            messages=[{"role": "system", "content": "사족 없이 팩트 위주 요약 리포트 작성."}],
...
def get_ai_expert_analysis(stock_name):
    if not client: return "분석 대기 중."
    try:
        prompt = (f"{stock_name}에 대해 사족 없이 [차트], [수급], [이슈] 핵심만 전문가답게 요약 보고하세요.")
        res = client.chat.completions.create(
            model="llama-3.3-70b-versatile",
            messages=[{"role": "system", "content": "냉철한 투자 전략가 모드."}],
    ...
    with col_main:
        ...
        with chart_col:
            ...
            try:
                ...
                # [핵심 수정] 데이터가 0인 노이즈 행 제거 (최근 일봉 오류 해결) [cite: 2026-01-16]
        ...
        else: st.info("분석할 만한 주요 기업 뉴스가 없습니다.")
