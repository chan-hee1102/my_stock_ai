...
if data is not None:
    ...
    with col_chat:
        ...
        # --- 핵심 수정 부분: 질문에 집중하는 프롬프트 ---
        if prompt := st.chat_input(f"{stock['종목명']}에 대해 질문하세요"):
            st.session_state.messages.append({"role": "user", "content": prompt})
            
            if client:
                try:
                    # AI에게 질문의 핵심에만 집중하도록 지시를 바꿨습니다.
                    instruction = (
                        f"당신은 {stock['종목명']}의 주식 전문가입니다. "
                        f"사용자의 질문에 대해 핵심만 짚어서 친절하게 대화하듯 답변하세요. "
                        f"질문과 관련 없는 긴 종목 분석 리포트를 반복해서 작성하지 마세요. "
                        f"만약 인사를 하면 인사를 하고, 짧은 질문에는 짧게 답변하세요."
                    )
                    ...
                    if response.text:
                        ...
                        st.session_state.messages.append({"role": "assistant", "content": "죄송합니다. 답변을 생성하지 못했습니다."})
