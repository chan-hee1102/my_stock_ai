...
# [v1.7 대응] 나스닥 선물 지수 포함 실시간 매크로 데이터 수집
...
def calculate_ai_probability(df, market_df):
    try:
        ...
        df['range_roc'] = ta.roc(df['price_range'], length=5) # v1.7 신규 피처
        df['day_of_week'] = df.index.dayofweek
        
        # 2. 실시간 매크로 데이터 병합 (나스닥 선물 포함)
        ...
        reasons = [
            {"label": "VIX 공포지수", "val": f"{v_cls:.1f}", "desc": "매우 안정" if v_cls < 18 else "안정"},
            ...
            {"label": "변동성 가속도", "val": f"{round(float(last['range_roc']), 1)}%", "desc": "에너지 폭발" if last['range_roc'] > 0 else "에너지 응축"}
        ]
        return round(prob, 1), "v1.7 초고도화 엔진 정상 작동 중", reasons
    except Exception as e: return 50.0, f"엔진 업데이트 중... ({str(e)})", []
